...
vcn_hub_dhcp_id="$(get_dhcp_id "$VCN_HUB_DHCP_NAME" "$vcn_hub_id")"
vcn_hub_subnprv_rt_id="$(get_route_table_id "$VCN_HUB_SUBNPRV_RT_NAME" "$vcn_hub_id")"
vcn_hub_subnprv_secl_id="$(get_secl_id "$VCN_HUB_SUBNPRV_SECL_NAME" "$vcn_hub_id")"

oci network subnet create \
    --compartment-id "$COMPARTMENT_ID" \
    --vcn-id "$vcn_hub_id" \
    --cidr-block "$VCN_HUB_SUBNPRV_CIDR" \
    --display-name "$VCN_HUB_SUBNPRV_NAME" \
    --dns-label "$VCN_HUB_SUBNPRV_DNS_NAME" \
    --dhcp-options-id "$vcn_hub_dhcp_id" \
    --route-table-id "$vcn_hub_subnprv_rt_id" \
    --security-list-ids "[\"$vcn_hub_subnprv_secl_id\"]" \
    --prohibit-public-ip-on-vnic "true" \
    --wait-for-state AVAILABLE
